...
env = jinja2.Environment(
    loader=jinja2.FileSystemLoader(Path(__file__).parent), enable_async=True
)
...
async def get_uid_by_name(name: str, cookies: dict) -> Optional[int]:
...
async def get_medals(uid: int, cookies: dict) -> List[dict]:
...
async def get_user_info(uid: int, cookies: dict) -> dict:
...
async def get_reply(name: str, cookies: dict) -> Union[str, bytes]:
    if name.isdigit():
        uid = int(name)
    else:
        try:
            uid = await get_uid_by_name(name, cookies)
            assert uid
        except Exception:
    ...
    try:
        user_info = await get_user_info(uid, cookies)
    except Exception:
        logger.warning(traceback.format_exc())
    ...
    template = env.get_template("info.html.jinja")
