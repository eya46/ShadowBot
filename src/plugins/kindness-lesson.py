# MIT https://github.com/zhaomaoniu/nonebot-plugin-kindness-lesson
...
async def generate(theme: str, key: str) -> str:
    resp = await api(
        model="qwen-turbo",
...
@matcher.handle()
async def _(args: Message = CommandArg(), key: str = GetValue("ali_ai_key")):
    if theme := args.extract_plain_text():
        try:
            await matcher.finish(await generate(theme, key))
        except MatcherException:
            raise
        except Exception as e:
            raise ActionError(f"生成恩情课文时出现错误：{e}") from e
...
async def got_theme(theme: str = ArgPlainText(), key: str = GetValue("ali_ai_key")):
    try:
        await matcher.finish(await generate(theme, key))
    except MatcherException:
        raise
